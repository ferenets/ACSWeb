...
using ACSWeb.Models;
using ACSWeb.Models.AccountViewModels;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    _logger.LogWarning("User account {0} locked out.", model.Email);
...
//***********************************************************************************************************************
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    _logger.LogInformation("User {0} created a new account with password.", model.UserName);

                    //var code = await _userManager.GenerateEmailConfirmationTokenAsync(user);
                    //var callbackUrl = Url.EmailConfirmationLink(user.Id, code, Request.Scheme);
                    //await _emailSender.SendEmailConfirmationAsync(model.Email, callbackUrl);

                   // await _signInManager.SignInAsync(user, isPersistent: false); //Попытка входа созданного пользователя
                    //_logger.LogInformation("User {0} created a new account with password and signed in", model.UserName);
                    return RedirectToLocal(returnUrl);
                }
                AddErrors(result);
            }
